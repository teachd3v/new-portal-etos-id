...
import { CalendarCheck, Loader2, Send, AlertTriangle, CheckCircle2, ShieldCheck, ArrowRight } from 'lucide-react';
import Link from 'next/link';
import toast from 'react-hot-toast';

export default function EtoserMonevBulananPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch('/api/etoser/monev-bulanan');
        const json = await res.json();
        
        if (!res.ok) {
          setErrorMsg(json.error || 'Terjadi kesalahan saat memuat data.');
        } else {
        ...
        setErrorMsg('Gagal memuat data instrumen monitoring.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const json = await res.json();
      if (json.success) {
        toast.success('Monitoring Bulanan berhasil disimpan!');
        setIsSubmitted(true);
      } else {
    ...
    return (
      <div className="max-w-[700px] mx-auto py-8 md:py-16 px-4 animate-in fade-in zoom-in-95 duration-500">
        <div className="bg-white/80 backdrop-blur-xl border border-emerald-200/80 rounded-2xl md:rounded-[2.5rem] p-6 sm:p-10 md:p-12 shadow-xl shadow-emerald-500/5 text-center relative overflow-hidden">
          ...
          <h2 className="text-xl sm:text-2xl md:text-3xl font-black text-slate-800 mb-2 tracking-tight">
            Capaian Bulan Ini Tercatat!
          </h2>

          <p className="text-slate-500 text-xs sm:text-sm md:text-base max-w-md mx-auto leading-relaxed mb-6">
            Terima kasih telah mengisi evaluasi mandiri bulanan periode <span className="font-bold text-sky-600">{activePeriod.bulan} {activePeriod.tahun}</span>.
          </p>
  ...
  return (
    <div className="max-w-[850px] mx-auto space-y-6 md:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-52 md:pb-32">
      
      {/* Header Section */}
      <div className="bg-gradient-to-br from-sky-600 to-indigo-700 rounded-2xl md:rounded-[2.5rem] p-4 md:p-10 text-white shadow-xl relative overflow-hidden">
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 md:gap-6">
          <div>
            <div className="inline-flex items-center gap-2 bg-white/20 backdrop-blur-md px-2.5 md:px-3 py-1 rounded-full text-[10px] md:text-xs font-bold uppercase tracking-wider mb-2 md:mb-3">
              <CalendarCheck className="w-3.5 h-3.5" /> Evaluasi Capaian Bulanan
            </div>
            <h1 className="text-2xl md:text-4xl font-black mb-1 md:mb-2 drop-shadow-sm">
              Monitoring Mandiri Etoser
            </h1>
            <p className="text-sky-100 font-medium text-xs md:text-lg max-w-xl">
              Evaluasi berkala kegiatan dan capaian diri untuk periode <span className="font-bold text-white">{activePeriod.bulan} {activePeriod.tahun}</span>.
            </p>
          </div>
        </div>
            ...
            return (
              <div key={q.id} className="space-y-3 md:space-y-4">
                <div className="flex gap-3 md:gap-4">
                  <div className="w-7 h-7 md:w-8 md:h-8 rounded-full bg-sky-50 text-sky-600 font-black flex items-center justify-center shrink-0 border border-sky-100 text-xs md:text-sm">
                    {idx + 1}
                  </div>
                  <div>
                    {q.indikator && (
                      <span className="text-[10px] md:text-xs font-bold text-sky-600 bg-sky-50 px-2 md:px-2.5 py-0.5 rounded-md mb-1 inline-block">
                        {q.indikator}
                      </span>
                    )}
                    <p className="font-bold text-slate-800 text-base md:text-lg leading-relaxed">
                      {q.item_pernyataan}
                    </p>
                  </div>
                </div>
                ...
                <div className="pl-0 sm:pl-11 space-y-3 md:space-y-4">
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-2.5 md:gap-3">
                    {[
                      ...
                      { value: 4, label: "Selalu", desc: "Sangat konsisten" }
                    ].map(option => (
                      <label 
                        key={option.value} 
                      ...
                      >
                        <input 
                          ...
                          className="sr-only"
                        />
                        <span className={`font-bold text-xs md:text-sm ${answers[q.kode] === option.value ? 'text-sky-800' : 'text-slate-700'}`}>
                          {option.label}
                        </span>
                        <span className="text-[10px] md:text-[11px] text-slate-400 mt-0.5">
                          {option.desc}
                        </span>
                      </label>
                    ))}
                  ...
                  {requiresValidation && answers[q.kode] !== undefined && (
                    <div className="mt-3 md:mt-4 animate-in fade-in zoom-in-95 duration-200">
                      <p className="text-xs md:text-sm font-bold text-sky-700 mb-1.5 md:mb-2">{q.pertanyaan_validasi}</p>
                      <textarea 
                        rows="2"
                        value={validations[q.kode] || ''}
                        onChange={(e) => handleValidationChange(q.kode, e.target.value)}
                        className="w-full px-3.5 py-2.5 md:px-4 md:py-3 bg-slate-50 border border-slate-200 rounded-xl text-xs md:text-sm focus:outline-none focus:ring-2 focus:ring-sky-500/20 text-slate-800"
                        placeholder="Tuliskan penjelasan atau bukti capaian Anda..."
                      ></textarea>
                    </div>
                  )}
                </div>
      ...
      <div className="fixed bottom-20 md:bottom-8 left-1/2 -translate-x-1/2 w-full max-w-[850px] px-3 md:px-4 z-40 md:z-50">
        <div className="bg-white/95 backdrop-blur-xl border border-slate-200/80 rounded-2xl md:rounded-[2rem] p-3 md:p-4 shadow-2xl flex flex-col sm:flex-row items-center gap-3 md:gap-6">
          <div className="flex-1 w-full px-1 md:px-4">
            <div className="flex justify-between items-end mb-1.5 md:mb-2">
              <span className="font-bold text-slate-700 text-xs md:text-sm">Progress Pengisian</span>
              <span className="font-black text-sky-600 text-xs md:text-sm">{progress}%</span>
            </div>
          ...
          <button 
            ...
            className={`flex items-center justify-center gap-1.5 md:gap-2 px-5 md:px-8 py-2.5 md:py-4 rounded-xl md:rounded-2xl font-bold text-xs md:text-sm transition-all shadow-sm shrink-0 w-full sm:w-auto ${
              ...
              ? 'bg-sky-600 hover:bg-sky-700 text-white shadow-sky-500/30 hover:shadow-lg hover:-translate-y-0.5' 
