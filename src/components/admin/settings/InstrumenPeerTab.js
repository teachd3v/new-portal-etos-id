...
export default function InstrumenPeerTab() {
  ...
  const fetchData = async () => {
    try {
      const res = await fetch('/api/admin/instrumen/peer');
  ...
  const handleSubmit = async (e) => {
    ...
    const url = isEdit ? `/api/admin/instrumen/peer/${formData.id}` : '/api/admin/instrumen/peer';
  ...
  const handleDelete = (id) => {
    setConfirmConfig({
      ...
      onConfirm: async () => {
        try {
          await fetch(`/api/admin/instrumen/peer/${id}`, { method: 'DELETE' });
          fetchData();
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        <h2 className="text-lg md:text-xl font-bold text-teal-950">Master Instrumen Peer</h2>
      ...
      <div className="hidden md:block bg-white/40 backdrop-blur-md rounded-2xl border border-white/60 overflow-x-auto">
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-teal-900/5">
            {loading ? <tr><td colSpan="10" className="p-8 text-center"><Loader2 className="w-6 h-6 animate-spin mx-auto text-teal-600" /></td></tr> : Array.isArray(data) ? data.map((row, i) => (
            ...
            )) : <tr><td colSpan="10" className="p-8 text-center text-teal-600">Gagal memuat data.</td></tr>}
          </tbody>
        </table>
      </div>
      ...
      <div className="block md:hidden space-y-3">
        {loading ? (
          ...
          <div className="p-8 text-center text-teal-800/60 font-medium bg-white/40 rounded-2xl">Belum ada data instrumen peer.</div>
        ) : (
          data.map((row, i) => {
        ...
        <div onClick={() => setIsModalOpen(false)} className="fixed inset-0 z-50 flex items-center justify-center bg-teal-950/20 backdrop-blur-sm p-4 cursor-pointer">
          <div onClick={(e) => e.stopPropagation()} className="bg-white/90 backdrop-blur-xl border border-white/60 rounded-2xl p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto cursor-default">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-bold text-teal-950">{formData.id ? 'Edit' : 'Tambah'} InstrumenPeer</h3>
      ...
      <CsvImportModal 
        ...
        endpoint="instrumen/peer" 
        templateHeaders={["variabel","kode","jenis_skala","item_pernyataan"]} 
        templateFileName="template_instrumen_peer.csv" 
      />
