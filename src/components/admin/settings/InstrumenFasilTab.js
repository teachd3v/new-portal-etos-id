...
export default function InstrumenFasilTab() {
  ...
  const fetchData = async () => {
    try {
      const res = await fetch('/api/admin/instrumen/fasil');
  ...
  const handleSubmit = async (e) => {
    ...
    const url = isEdit ? `/api/admin/instrumen/fasil/${formData.id}` : '/api/admin/instrumen/fasil';
  ...
  const handleDelete = (id) => {
    setConfirmConfig({
      ...
      onConfirm: async () => {
        try {
          await fetch(`/api/admin/instrumen/fasil/${id}`, { method: 'DELETE' });
          fetchData();
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        <h2 className="text-lg md:text-xl font-bold text-teal-950">Master Instrumen Fasil</h2>
          ...
          <tbody className="divide-y divide-teal-900/5">
            {loading ? <tr><td colSpan="10" className="p-8 text-center"><Loader2 className="w-6 h-6 animate-spin mx-auto text-teal-600" /></td></tr> : data.map((row, i) => (
              <tr key={i} className="hover:bg-white/40">
                ...
                <td className="p-4 text-sm text-teal-900">{row.role}</td>
      ...
      <div className="block md:hidden space-y-3">
        {loading ? (
          ...
          <div className="p-8 text-center text-teal-800/60 font-medium bg-white/40 rounded-2xl">Belum ada data instrumen fasil.</div>
        ) : (
          data.map((row, i) => {
        ...
        <div onClick={() => setIsModalOpen(false)} className="fixed inset-0 z-50 flex items-center justify-center bg-teal-950/20 backdrop-blur-sm p-4 cursor-pointer">
          <div onClick={(e) => e.stopPropagation()} className="bg-white/90 backdrop-blur-xl border border-white/60 rounded-2xl p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto cursor-default">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-bold text-teal-950">{formData.id ? 'Edit' : 'Tambah'} InstrumenFasil</h3>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <input type="text" required value={formData.role || ''} onChange={e => setFormData({...formData, role: e.target.value})} className="w-full px-4 py-2 rounded-xl bg-white/50 border border-teal-100 focus:outline-none focus:ring-2 focus:ring-teal-500/30" />
              </div>
              <div>
